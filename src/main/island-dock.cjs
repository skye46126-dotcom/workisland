 ...
 */
function dockGeometry(display, edge, offset) {
  ...
  if (edge === "top") {
    ...
    const stripX = Math.max(b.x, Math.min(b.x + Math.round((b.width - DOCK.TOP_W) * offset), b.x + b.width - DOCK.TOP_W));
    return {
      bounds: { x: winX, y: b.y, width: w, height: DOCK.TOP_WINDOW_H },
      strip: { spanOffset: stripX - winX, len: DOCK.TOP_W, depth: DOCK.TOP_H }
    };
  }
  const w = DOCK.VPANEL_W;
  const h = DOCK.VPANEL_H;
  const stripCenter = wa.y + offset * wa.height;
  const stripTop = Math.max(wa.y, Math.min(Math.round(stripCenter - DOCK.STRIP_H / 2), wa.y + wa.height - DOCK.STRIP_H));
  ...
  return {
    ...
    strip: { spanOffset: stripTop - winY, len: DOCK.STRIP_H, depth: DOCK.STRIP_W }
  };
}
...
function createDockableIslandWindow(BaseIslandWindow, deps) {
  ...
  class DockableIslandWindow extends BaseIslandWindow {
    ...
    dockGeometry() {
      return dockGeometry(this.currentTarget.display, this._dockEdge, this._dockOffset);
    }
...
module.exports = { DOCK, dockGeometry, nearestEdge, createDockableIslandWindow, attachIslandDock };
